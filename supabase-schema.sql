-- New Store 8 POS — Supabase database schema.
--
-- This runs INSIDE THE SAME Supabase project as every other client (no new
-- project, no extra cost). Every table/function/trigger/index for this
-- client is suffixed "_sp08" so it lives completely separate from every
-- other client's tables in the same database — different rows, different
-- RPC functions, zero overlap. Paste this whole file into the SAME
-- project's SQL Editor and click Run; it only creates new "_sp08"
-- objects, it does not touch any other client's tables.
--
...
-- privileges but only do exactly what they're written to do.

-- ============== TABLES ==============


create table if not exists items_sp08 (
  id text primary key,
  name text not null,
...
alter table items_sp08 add column if not exists cost numeric default 0;
alter table items_sp08 add column if not exists expiry text default '';

-- A master reference catalog — items_sp08 here are NOT live inventory (no stock
-- tracking, don't show up in Billing/Purchases) until explicitly copied into
-- the `items_sp08` table via "Add to Inventory". Lets a store owner import a huge
-- premade product list without it looking like they stock everything in it.
create table if not exists item_catalog_sp08 (
  id text primary key,
  name text not null,
  barcode text default '',
  category text default '',
...
-- Safe to re-run — no-op if the column already exists.
alter table item_catalog_sp08 add column if not exists brand text default '';
alter table item_catalog_sp08 add column if not exists cost numeric default 0;

create table if not exists categories_sp08 (
  name text primary key
);

create table if not exists suppliers_sp08 (
  id text primary key,
  name text not null,
...
create table if not exists cashiers_sp08 (
  id text primary key,
  name text not null,
  pin text default ''
);

create table if not exists purchases_sp08 (
  id text primary key,
  date date,
  supplier_id text,
  supplier_name text,
...
-- General business expenses_sp08 (rent, utilities, salaries, misc) — "money out"
-- that ISN'T buying inventory stock (that's what `purchases_sp08` already tracks).
-- Powers the Financials tab's Money In vs Money Out view.
create table if not exists expenses_sp08 (
  id text primary key,
...
-- Safe to re-run — no-op if the column already exists.
alter table expenses_sp08 add column if not exists period text default '';

-- Udhaar/credit accounts — a regular customer buys now, pays later.
-- `balance` is a running counter (like items_sp08.stock), updated directly by the
-- app on every credit sale (+) and logged payment (-), not recomputed from
-- sales_sp08 history — keeps this consistent with how stock is already tracked.
create table if not exists customers_sp08 (
  id text primary key,
  name text not null,
  phone text default '',
  notes text default '',
...
-- Ledger of payments customers_sp08 make against their credit balance — kept
-- separate from `customers_sp08` itself so there's a printable/auditable history.
create table if not exists customer_payments_sp08 (
  id text primary key,
  customer_id text,
...
create table if not exists sales_sp08 (
  id text primary key,
  receipt_no integer,
  date timestamptz,
  customer text,
...
create table if not exists sale_lines_sp08 (
  sale_id text,
  item_id text,
  item_name text,
  barcode text,
...
create table if not exists refunds_sp08 (
  id text primary key,
  sale_id text,
  receipt_no integer,
  date timestamptz,
...
create table if not exists refund_lines_sp08 (
  refund_id text,
  item_id text,
  item_name text,
  qty numeric,
...
create table if not exists held_sales_sp08 (
  id text primary key,
  date timestamptz,
  customer text,
  discount numeric,
...
create table if not exists held_sales_cart_sp08 (
  held_id text,
  item_id text,
  qty numeric
);

create table if not exists shifts_sp08 (
  id text primary key,
  cashier_name text,
  start timestamptz,
  "end" timestamptz,
...
create table if not exists active_shift_sp08 (
  id text primary key,
  cashier_id text,
  cashier_name text,
  start timestamptz,
...
create table if not exists settings_sp08 (
  key text primary key,
  value text
);

create table if not exists meta_sp08 (
  key text primary key,
  value text
);

-- Running totals, kept up to date by triggers below (not recomputed on
-- read). "All Time" stats/top-items_sp08 come from these instead of scanning the
-- full sales_sp08/sale_lines_sp08 tables — the live scan approach that used to time
-- out once sales_sp08 history got into the tens of thousands. Any bounded range
...
create table if not exists item_sales_totals_sp08 (
  item_name text primary key,
  qty numeric default 0,
  revenue numeric default 0
);

create table if not exists sales_overall_totals_sp08 (
  key text primary key,
  count integer default 0,
  revenue numeric default 0
);
insert into sales_overall_totals_sp08 (key, count, revenue) values ('all', 0, 0)
...
alter table items_sp08 enable row level security;
alter table item_catalog_sp08 enable row level security;
alter table categories_sp08 enable row level security;
alter table suppliers_sp08 enable row level security;
alter table cashiers_sp08 enable row level security;
alter table purchases_sp08 enable row level security;
alter table expenses_sp08 enable row level security;
alter table sales_sp08 enable row level security;
alter table sale_lines_sp08 enable row level security;
alter table refunds_sp08 enable row level security;
alter table refund_lines_sp08 enable row level security;
alter table held_sales_sp08 enable row level security;
alter table held_sales_cart_sp08 enable row level security;
alter table shifts_sp08 enable row level security;
alter table active_shift_sp08 enable row level security;
alter table settings_sp08 enable row level security;
alter table meta_sp08 enable row level security;
alter table item_sales_totals_sp08 enable row level security;
alter table sales_overall_totals_sp08 enable row level security;
alter table customers_sp08 enable row level security;
alter table customer_payments_sp08 enable row level security;

-- ============== triggers: keep running totals in sync ==============
-- Fire on every insert/update/delete to sales_sp08/sale_lines_sp08, regardless of
...
create or replace function trg_sale_lines_totals_sp08()
returns trigger
language plpgsql
security definer
...
begin
  if TG_OP = 'INSERT' then
    insert into item_sales_totals_sp08 (item_name, qty, revenue)
    values (NEW.item_name, NEW.qty, NEW.subtotal)
    on conflict (item_name) do update set
      qty = item_sales_totals_sp08.qty + NEW.qty,
      revenue = item_sales_totals_sp08.revenue + NEW.subtotal;
    return NEW;
  elsif TG_OP = 'DELETE' then
    update item_sales_totals_sp08 set qty = qty - OLD.qty, revenue = revenue - OLD.subtotal
...
drop trigger if exists sale_lines_totals_trigger_sp08 on sale_lines_sp08;
create trigger sale_lines_totals_trigger_sp08
after insert or delete on sale_lines_sp08
for each row execute function trg_sale_lines_totals_sp08();

create or replace function trg_sales_totals_sp08()
returns trigger
language plpgsql
security definer
set search_path = public
...
begin
  if TG_OP = 'INSERT' then
    update sales_overall_totals_sp08 set count = count + 1, revenue = revenue + NEW.grand where key = 'all';
    return NEW;
  elsif TG_OP = 'DELETE' then
    update sales_overall_totals_sp08 set count = count - 1, revenue = revenue - OLD.grand where key = 'all';
    return OLD;
  elsif TG_OP = 'UPDATE' then
    update sales_overall_totals_sp08 set revenue = revenue - OLD.grand + NEW.grand where key = 'all';
    return NEW;
  end if;
  return null;
end;
...
drop trigger if exists sales_totals_trigger_sp08 on sales_sp08;
create trigger sales_totals_trigger_sp08
after insert or update or delete on sales_sp08
for each row execute function trg_sales_totals_sp08();
...
insert into item_sales_totals_sp08 (item_name, qty, revenue)
select item_name, sum(qty), sum(subtotal) from sale_lines_sp08 group by item_name
on conflict (item_name) do update set qty = excluded.qty, revenue = excluded.revenue;

update sales_overall_totals_sp08
set count = (select count(*) from sales_sp08), revenue = (select coalesce(sum(grand), 0) from sales_sp08)
where key = 'all';

-- ============== sync_push_sp08: atomic full-replace write ==============

create or replace function sync_push_sp08(payload jsonb)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  if payload ? 'itemCatalog' then
    delete from item_catalog_sp08 where true;
    insert into item_catalog_sp08 (id, name, barcode, category, brand, price, cost, unit, low_stock)
  ...
  if payload ? 'customerPayments' then
    delete from customer_payments_sp08 where true;
    insert into customer_payments_sp08 (id, customer_id, customer_name, amount, date, notes)
  ...
  if payload ? 'sales' then
    ...
    truncate sale_lines_sp08;
    truncate sales_sp08;
    truncate item_sales_totals_sp08;
    update sales_overall_totals_sp08 set count = 0, revenue = 0 where key = 'all';

    insert into sales_sp08 (id, receipt_no, date, customer, cashier, payment, cash, subtotal, discount_pct, discount_amt, tax_pct, tax_amt, grand)
  ...
  if payload ? 'heldSales' then
    delete from held_sales_cart_sp08 where true;
  ...
  if payload ? 'receiptCounter' then
    insert into meta_sp08 (key, value) values ('receiptCounter', (payload->>'receiptCounter'))
    on conflict (key) do update set value = excluded.value;
  end if;
end;
$$;

-- ============== sync_pull_sp08: read everything EXCEPT sales_sp08/refunds_sp08 ==============
-- Sales and refunds_sp08 are deliberately left out here — that history can grow
-- without bound, and building/sending all of it on every app load is what
-- broke this at high volume. Sales/refunds_sp08 are fetched separately, scoped to
-- whatever date range the user has selected, by sync_pull_sales_range_sp08 below.

create or replace function sync_pull_sp08()
returns jsonb
language plpgsql
security definer
set search_path = public
...
begin
  select jsonb_build_object(
    ...
    )) from customer_payments_sp08), '[]'::jsonb),
    ...
    'heldSales', coalesce((select jsonb_agg(
      jsonb_build_object(
        ...
        )) from held_sales_cart_sp08 hc where hc.held_id = h.id), '[]'::jsonb)
      )
...
-- ============== indexes for range-scoped sales_sp08/refunds_sp08 lookups ==============

create index if not exists idx_sales_date_sp08 on sales_sp08(date);
create index if not exists idx_refunds_date_sp08 on refunds_sp08(date);
create index if not exists idx_sale_lines_sale_id_sp08 on sale_lines_sp08(sale_id);
create index if not exists idx_refund_lines_refund_id_sp08 on refund_lines_sp08(refund_id);

-- ============== sync_pull_sales_range_sp08: one page of sales_sp08/refunds_sp08 for a date range ==============
...
--   - sales_sp08/refunds_sp08: one page (page_size rows, newest-first) — pass
...
--     LATERAL join so only the selected page's line items_sp08 get aggregated,
--     never the whole sale_lines_sp08 table, however large it's grown.
...
--     once sales_sp08 history got into the tens of thousands.
-- Pass null for range_start/range_end for an unbounded (All Time) range.

create or replace function sync_pull_sales_range_sp08(
...
begin
  if is_all_time then
    select count, revenue into v_total_count, v_total_revenue from sales_overall_totals_sp08 where key = 'all';
    select coalesce(jsonb_agg(jsonb_build_object('name', item_name, 'qty', qty, 'revenue', revenue) order by qty desc), '[]'::jsonb)
    into v_top_items
    from (select * from item_sales_totals_sp08 order by qty desc limit 20) t;
  else
    select count(*) into v_total_count from sales_sp08
    where (range_start is null or date >= range_start) and (range_end is null or date <= range_end);

    select coalesce(sum(grand), 0) into v_total_revenue from sales_sp08
    where (range_start is null or date >= range_start) and (range_end is null or date <= range_end);
    ...
    from (
      ...
      from sale_lines_sp08 sl
      join sales_sp08 s on s.id = sl.sale_id
  ...
  -- pre-aggregated the ENTIRE sale_lines_sp08 table before the join happened, so
  ...
  left join lateral (
    ...
    from sale_lines_sp08 where sale_id = t.id
  ) sl on true;
    ...
    'refunds', coalesce((
      select jsonb_agg(x order by x->>'date' desc) from (
        ...
        left join lateral (
          ...
          from refund_lines_sp08 where refund_id = r.id
        ) rl on true
...
-- ============== sync_append_sale_sp08: instant single-sale insert ==============
-- Used right after checkout, on every device, so a sale reaches the database
-- immediately without resending the entire sales_sp08 history each time. Safe to
-- retry (e.g. after a dropped connection) since it upserts on the sale's id.

create or replace function sync_append_sale_sp08(sale jsonb, receipt_counter integer default null)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  delete from sale_lines_sp08 where sale_id = sale->>'id';
  insert into sale_lines_sp08 (sale_id, item_id, item_name, barcode, price, qty, unit, subtotal)
  ...
  if receipt_counter is not null then
    insert into meta_sp08 (key, value) values ('receiptCounter', receipt_counter::text)
    on conflict (key) do update set value = excluded.value;
  end if;
end;
$$;

-- ============== sync_append_sales_batch_sp08: bulk append, no wipe ==============
-- Set-based insert for many sales_sp08 at once (e.g. bulk-loading historical data)
-- without ever deleting the existing table first, unlike sync_push_sp08. Callers
-- should chunk large loads (a few thousand sales_sp08 per call) to stay under
...
-- ============== sync_append_refund_sp08: instant single-refund insert ==============

create or replace function sync_append_refund_sp08(refund jsonb)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  delete from refund_lines_sp08 where refund_id = refund->>'id';
  insert into refund_lines_sp08 (refund_id, item_id, item_name, qty, price, refund_amount)
...
-- ============== sync_clear_sales_sp08: wipes sales_sp08/sale_lines_sp08 only ==============
-- Used by "Clear all sales_sp08 history" — a deliberate one-off admin action,
-- not something that happens per checkout.

create or replace function sync_clear_sales_sp08()
returns void
language plpgsql
security definer
set search_path = public
...
-- Every button that adds/edits/deletes items_sp08, categories_sp08, suppliers_sp08,
-- purchases_sp08, cashiers_sp08, held sales_sp08, shifts_sp08, or settings_sp08 calls one of these
...
create or replace function sync_replace_item_catalog_sp08("itemCatalog" jsonb)
returns void
language plpgsql
security definer
set search_path = public
...
create or replace function sync_replace_customer_payments_sp08("customerPayments" jsonb)
returns void
language plpgsql
security definer
set search_path = public
...
grant execute on function sync_push_sp08(jsonb) to anon;
grant execute on function sync_pull_sales_range_sp08(timestamptz, timestamptz, integer, timestamptz, text) to anon;
...
drop function if exists sync_pull_sales_range_sp08(timestamptz, timestamptz, integer);
grant execute on function sync_pull_sp08() to anon;
grant execute on function sync_append_sale_sp08(jsonb, integer) to anon;
grant execute on function sync_append_sales_batch_sp08(jsonb) to anon;
grant execute on function sync_append_refund_sp08(jsonb) to anon;
grant execute on function sync_clear_sales_sp08() to anon;
grant execute on function sync_replace_items_sp08(jsonb) to anon;
grant execute on function sync_replace_item_catalog_sp08(jsonb) to anon;
grant execute on function sync_replace_categories_sp08(jsonb) to anon;
grant execute on function sync_replace_suppliers_sp08(jsonb) to anon;
grant execute on function sync_replace_cashiers_sp08(jsonb) to anon;
grant execute on function sync_replace_purchases_sp08(jsonb) to anon;
grant execute on function sync_replace_expenses_sp08(jsonb) to anon;
grant execute on function sync_replace_customers_sp08(jsonb) to anon;
grant execute on function sync_replace_customer_payments_sp08(jsonb) to anon;
grant execute on function sync_replace_held_sales_sp08(jsonb) to anon;
grant execute on function sync_replace_shifts_sp08(jsonb) to anon;
grant execute on function sync_replace_active_shift_sp08(jsonb) to anon;
grant execute on function sync_replace_settings_sp08(jsonb) to anon;
...
-- ('pos-spare-08' or 'client-manager') so their credentials are
...
insert into app_logins (app_name, username, password_hash)
select 'pos-spare-08', 'admin', extensions.crypt('changeme123', extensions.gen_salt('bf'))
where not exists (select 1 from app_logins where app_name = 'pos-spare-08');
...
insert into admin_settings_passwords (app_name, password_hash)
select 'pos-spare-08', extensions.crypt('changeme456', extensions.gen_salt('bf'))
where not exists (select 1 from admin_settings_passwords where app_name = 'pos-spare-08');
